...
- **Firewall redirect** — sets dnsmasq `port=0` (dnsmasq no longer listens on `:53`) and redirects LAN port 53 (TCP/UDP) to Mihomo in netfilter. Catches hard-coded public DNS (e.g. `8.8.8.8:53`). Keep `dns.listen` on `0.0.0.0:7874` in `config.yaml`. Redirect applies on all LAN ingress (VLAN and bridge member ports included); WAN is excluded.
